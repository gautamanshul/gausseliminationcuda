...
2. Solves the same system on the GPU using a two-kernel CUDA pipeline (`compute_scale_factors_kernel` for scaling, `gaussian_elimination_kernel` for parallel elimination with sequential per-pivot row swap), and times the solve.
3. Validates correctness via Google Test (`ASSERT_NEAR` between CPU and GPU solution vectors with tolerance 1e-5).
...
3. **Single optimization variant.** Naive global-memory kernel only — no shared-memory tiling, no loop unrolling, no multi-kernel pipelining, no parallel back-substitution. **RQ2** of the proposal addresses this systematically with an ablation.
